...
<template>
  <div v-if="userInfo._id" class="chat">
    <!--头部区域-->
    <div class="header">
        ...
        <div class="shopCart_list_con" v-for="(goods,index) in cartGoods" :key="index">
          <div class="list_con_left">
            <a href="javascript:;"
               ...
               @click.stop="singerGoodsSelect(goods)"
            ></a>
          </div>
          <div class="list_con_right">
            ...
            <div class="shop_con">
              ...
              <p class="shop_price">{{goods.price * goods.buy_count | moneyFormat}}</p>
              <div class="shop_deal">
    ...
    <div id="tab_bar">
      <div class="tab-bar-left">
        <a href="javascript:;"
           class="cart_check_box"
           :checked="isSelectAll"
           @click.stop="selectedAll(isSelectAll)"
        ></a>
        <span style="font-size: 16px;">全选</span>
        <div class="select-all">
          合计：<span class="total-price"><!--&yen;999.00-->{{totalPrice | moneyFormat}}</span>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 2.单个商品的选中和取消选中
      singerGoodsSelect(goods){
           this.$store.dispatch('singerGoodsSelect',{goods});
          // 3.2判断是否全部选中
           this.hasSelectAll();
           // 4.1计算所有选中商品的总价格
           this.computedTotalPrice()
      ...
      selectedAll(isSelect){
          ...
          this.$store.dispatch('allGoodsSelect',{isSelect});
        // 4.1计算所有选中商品的总价格
          this.computedTotalPrice()
      },
      // 3.2判断当点击所有购物车单个商品时的全选
      hasSelectAll(){
         let flag = true ;
      ...
      // 删除购物车商品
       clickTrash(goods){
         MessageBox.confirm('确定删除改商品吗？').then(action =>{
              if(action === 'confirm'){
